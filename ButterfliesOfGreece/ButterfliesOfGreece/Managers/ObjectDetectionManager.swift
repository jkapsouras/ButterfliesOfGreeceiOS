...
class DetectionModelDataHandler: NSObject {
	...
	/// results for a successful inference.
	func runModel(onFrame pixelBuffer: CVPixelBuffer) -> DetectionResult? {
		...
		let result = DetectionResult(inferenceTime: interval, inferences: resultArray)
		return result
	}
	
	/// Filters out all the results with confidence score < threshold and returns the top N results
	/// sorted in descending order.
	func formatResults(boundingBox: [Float], outputClasses: [Float], outputScores: [Float], outputCount: Int, width: CGFloat, height: CGFloat) -> [DetectionInference]{
		var resultsArray: [DetectionInference] = []
		if (outputCount == 0) {
			return resultsArray
		}
		for i in 0...outputCount - 1 {
			...
			let inference = DetectionInference(confidence: score,
